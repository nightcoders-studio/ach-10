...
4. API Routes
GET/POST Harga
typescript// app/api/prices/route.ts
...
5. Komponen Utama
Form Submit Harga
typescript// components/PriceForm.tsx
'use client'
...
ChatWidget
typescript// components/ChatWidget.tsx
'use client'
...
Buka dashboard → tampilkan tabel harga cabai merah dari 3 pasar berbeda
Klik link Google Maps salah satu lokasi
Buka form → input harga baru secara live
Refresh dashboard → data langsung muncul (realtime)
Buka chatbot → tanya "cabai paling murah di mana sekarang?" → AI jawab dengan lokasi + link maps
Tanya "beras ramos di Pasar Peunayong berapa?" → AI jawab spesifik
